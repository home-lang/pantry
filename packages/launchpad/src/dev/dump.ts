...
export async function dump(dir: string, options: DumpOptions = {}): Promise<void> {
  ...
  try {
    ...
    if (shellOutput && isEnvironmentReady) {
      ...
      console.log(`fi`)
      console.log(`# Ensure we have basic system paths if PATH is empty or already contains launchpad paths`)
      console.log(`if [[ -z "$LAUNCHPAD_ORIGINAL_PATH" || "$LAUNCHPAD_ORIGINAL_PATH" =~ "launchpad" ]]; then`)
