...
import specimen, { _internals } from './sniff.ts'

export const fixturesd = new Path(new URL(import.meta.url).pathname).parent().parent().join('fixtures')
  ...
  try {
      ...
      for (const [keyfile, dep] of keyfiles) {
        await test.step(`${keyfile}`, async () => {
          const go = async (file: Path) => {
            ...
            assert(
              pkgs.find(pkg => utils.pkg.str(pkg) == 'zlib.net^1.2'),
              'should dep zlib^1.2',
            )
            if (dep) {
              assert(
                pkgs.find(pkg => utils.pkg.str(pkg) == dep),
                `should dep ${dep}`,
              )
            }
    ...
    await runner.step('fixed fixtures', async (test) => {
      const keyfiles = [
        ...
        ['bun.lockb', 'bun.sh>=1'],
        [
          'pyproject.toml/poetry-yaml-fm/pyproject.toml',
          'pip.pypa.io',
          'python~3.10',
        ],
    ...
    await runner.step('broken .python-version', async () => {
      const file = fixturesd.join('python-version/broken/.python-version').cp({
        into: Path.mktemp(),
      })
    ...
    await runner.step('empty action.yml has no deps', async () => {
      ...
      const { pkgs: pkgs2 } = await specimen(
        fixturesd.join('action.yml/not-node'),
      )
    ...
    await runner.step('skips invalid deps node', async () => {
      const f = Path.mktemp().join('package.json').rm().write({
        text: '{"pkgx": {"dependencies": true}}',
      })
    ...
    await runner.step('skffold.yaml', async () => {
      ...
      assert(
        pkgs.length === 1,
        'invalid skaffold.yaml should not return any dep',
      )
      ...
      for (const [keyfile, ...deps] of keyfiles) {
        ...
        const { env, pkgs } = await specimen(file.parent())
        assert(
          pkgs.length === deps.length,
          `dependencies length differ, required: ${deps.length}, actual: ${pkgs.length}`,
        )
        deps.every((dep) => {
          assert(
            pkgs.find(pkg => utils.pkg.str(pkg) == dep),
