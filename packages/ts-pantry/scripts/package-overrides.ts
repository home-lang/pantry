...
export const packageOverrides: Record<string, PackageOverride> = {
  ...
  'sourceforge.net/xmlstar': {
    ...
    modifyRecipe: (recipe: NormalizedRecipe) => {
      ...
      // Clear CFLAGS/CPPFLAGS from recipe env — they reference {{deps.gnome.org/libxml2.prefix}}
      // which won't resolve since we removed the dep. Our prependScript sets correct paths.
      if (recipe.build?.env?.CFLAGS) delete recipe.build.env.CFLAGS
      if (recipe.build?.env?.CPPFLAGS) delete recipe.build.env.CPPFLAGS
    },
  },
  ...
  'videolan.org/x265': {
    ...
    // Delete any cmake_policy lines setting OLD behavior — cmake 4.x defaults to NEW.
  ...
  'gnu.org/guile': {
    ...
    modifyRecipe: (recipe: NormalizedRecipe) => {
      ...
      // Fix scmconfig.h circular dependency: gen-scmconfig.c includes scm.h which
      // includes scmconfig.h, but scmconfig.h is generated by compiling gen-scmconfig.c.
      // Bootstrap by copying configure's gen-scmconfig.h as scmconfig.h before make.
      const hwConcRe = /\{\{\s*hw\.concurrency\s*\}\}/g
      if (Array.isArray(recipe.build?.script)) {
        // Insert scmconfig.h bootstrap step after ./configure
        for (let i = 0; i < recipe.build.script.length; i++) {
          const step = recipe.build.script[i]
          if (typeof step === 'string' && step.includes('./configure')) {
            recipe.build.script.splice(i + 1, 0,
              'cp libguile/gen-scmconfig.h libguile/scmconfig.h 2>/dev/null || touch libguile/scmconfig.h',
            )
            break
          }
        }
        // Split "make install" into "make" + "make install" (single-threaded)
        for (let i = 0; i < recipe.build.script.length; i++) {
          const step = recipe.build.script[i]
    ...
    modifyRecipe: (recipe: NormalizedRecipe) => {
      ...
      if (recipe.build) {
        ...
        recipe.build.script = [
          [
            'if test "{{hw.platform}}" = "darwin"; then',
            '  # macOS: extract the .pkg installer (no sudo needed)',
            '  curl -fSL "https://awscli.amazonaws.com/AWSCLIV2.pkg" -o awscli.pkg',
            '  pkgutil --expand-full awscli.pkg awscli-expanded',
            '  # Find the Payload dir containing the aws binary tree',
            '  PAYLOAD_DIR=$(find awscli-expanded -type d -name Payload | head -1)',
            '  AWS_BIN=$(find "$PAYLOAD_DIR" -name "aws" -not -name "*.py" -type f 2>/dev/null | head -1)',
            '  if test -z "$AWS_BIN"; then',
            '    echo "ERROR: could not find aws binary in expanded pkg"',
            '    find awscli-expanded -maxdepth 5 -type f | head -30',
            '    exit 1',
            '  fi',
            '  # Copy the entire aws-cli tree from the Payload',
            '  AWS_ROOT=$(dirname "$AWS_BIN")',
            '  # Walk up to find the top-level aws-cli directory',
            '  while test "$(basename "$AWS_ROOT")" != "aws-cli" && test "$AWS_ROOT" != "/"; do',
            '    AWS_ROOT=$(dirname "$AWS_ROOT")',
            '  done',
            '  mkdir -p "{{prefix}}/bin"',
            '  cp -R "$AWS_ROOT" "{{prefix}}/aws-cli"',
            '  # Create bin symlinks — find the aws binary relative to prefix',
            '  INSTALLED_BIN=$(find "{{prefix}}/aws-cli" -name "aws" -not -name "*.py" -type f | head -1)',
            '  REL_PATH=$(echo "$INSTALLED_BIN" | sed "s|{{prefix}}/||")',
            '  ln -sf "../${REL_PATH}" "{{prefix}}/bin/aws"',
            '  # aws_completer lives next to aws',
            '  COMPLETER_DIR=$(dirname "$INSTALLED_BIN")',
            '  if test -f "$COMPLETER_DIR/aws_completer"; then',
            '    COMPL_REL=$(echo "$COMPLETER_DIR/aws_completer" | sed "s|{{prefix}}/||")',
            '    ln -sf "../${COMPL_REL}" "{{prefix}}/bin/aws_completer"',
            '  fi',
            'else',
            '  # Linux: download the official zip bundle',
    ...
    platforms: {
      darwin: {
        prependScript: [
          // Unlink Homebrew boost so cmake uses S3 boost (matching folly's soname)
          'brew unlink boost 2>/dev/null || true',
          // cc_wrapper now uses -Wl,-ld_classic to avoid "duplicate linked dylib" errors
        ],
      },
      linux: {
        prependScript: [
  ...
  'seaweedfs.com': {
    modifyRecipe: (recipe: NormalizedRecipe) => {
      // Recipe pins go.dev: =1.22.0 but project requires 1.24+
      if (recipe.build?.dependencies?.['go.dev']) {
        recipe.build.dependencies['go.dev'] = '*'
      }
    },
  },

  // ─── spacetimedb.com — download pre-built binaries from GitHub releases ──
      ...
      if (recipe.build) {
        ...
        recipe.build.script = [
          [
            ...
            '  # macOS: embedded/bin/shards, linux: bin/shards',
            '  SHARDS=$(find /tmp/crystal-dl -name shards -type f 2>/dev/null | head -1)',
            'fi',
            'mkdir -p "{{prefix}}/bin"',
  ...
  'ruby-lang.org': {
    platforms: {
      darwin: {
        // Ruby's configure LDFLAGS check uses -Werror. On Xcode 26 the linker warns
        // about deprecated -ld_classic and LIBRARY_PATH dirs that don't exist (build
        // deps like patch/autoconf have no lib/). -Wl,-w suppresses linker warnings.
        prependScript: ['export LDFLAGS="-Wl,-w"'],
      },
    },
    modifyRecipe: (recipe: NormalizedRecipe) => {
      ...
      if (recipe.build) {
        ...
        recipe.build.script = [
          [
            ...
            '  [[ "$SELF" != /* ]] && SELF="$DIR/$SELF"',
            'done',
            'REAL_DIR="$(cd "$(dirname "$SELF")/../libexec/mole" && pwd)"',
            'exec "$REAL_DIR/mole" "$@"',
            'WRAPPER',
