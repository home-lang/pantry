...
export const recipe: Recipe = {
  propsDir: 'props/aspell.net',
  domain: 'aspell.net',
  name: 'aspell',
  description: 'Spell checker with better logic than ispell',
  homepage: 'https://aspell.net/',
  programs: ['aspell', 'aspell-import', 'precat', 'preunzip', 'prezip', 'prezip-bin', 'pspell-config', 'run-with-aspell', 'word-list-compress'],
  distributable: {
    ...
    script: [
      {
        run: [
          ...
          '# NOTE: aspell 0.60.8.2 does not recognize --with-compiledby (configure',
          '# only warns and ignores it). Match upstream pkgx/Homebrew: plain prefix.',
          ...
          'for url in "${urls[@]}"; do',
          '  # Dictionaries are OPTIONAL extras — never let a flaky download or a',
          '  # single dict build abort the core aspell install. ftp.gnu.org is',
          '  # unreliable from CI, so go through the ftpmirror redirector with',
          '  # retries and treat any failure of an individual dict as skippable.',
          '  (',
          '    set -e',
          '    curl --fail --location --retry 5 --retry-all-errors --connect-timeout 30 \\',
          '      -O "https://ftpmirror.gnu.org/gnu/aspell/dict/$url.tar.bz2"',
          ...
          '  ) || echo "WARN: aspell dictionary $url unavailable, skipping"',
          'done',
          '# Dictionaries are optional extras; ensure the loop never leaves a',
          '# non-zero status that the script-level `set -e` would abort on.',
          'true',
        ].join('\n'),
      },
    ],
