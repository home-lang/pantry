...
export const recipe: Recipe = {
  ...
  build: {
    script: [
      ...
      // The installed abslTargets*.cmake bake in the absolute install prefix.
      // Rewrite it to a path relative to the file's own location so the package
      // is relocatable. These files live at {{prefix}}/lib/cmake/absl/, so
      // {{prefix}} is exactly 3 levels up (absl -> cmake -> lib -> prefix).
      'sed -i -e "s:{{prefix}}:\\$\\{CMAKE_CURRENT_LIST_DIR\\}/../../..:g" -e "s/\\+brewing//g" abslTargets{,-release}.cmake',
    ],
    env: {
