...
pub fn shimCommand(allocator: std.mem.Allocator, packages: []const []const u8, options: ShimOptions) !CommandResult {
    ...
    try std.Io.Dir.cwd().makePath(shim_dir);
...
fn createShim(
    ...
    if (!options.force) {
        std.Io.Dir.cwd().access(shim_path, .{}) catch |err| {
        ...
        if (std.Io.Dir.cwd().access(shim_path, .{})) |_| {
    ...
    // Write shim file
    var file = try std.Io.Dir.cwd().createFile(shim_path, .{});
    defer file.close();

    try file.writeAll(shim_content);
...
pub fn shimListCommand(allocator: std.mem.Allocator) !CommandResult {
    ...
    var dir = std.Io.Dir.cwd().openDir(shim_dir, .{ .iterate = true }) catch |err| {
        if (err == error.FileNotFound) {
    ...
    while (try iter.next()) |entry| {
        if (entry.kind == .file) {
            ...
            const content = std.Io.Dir.cwd().readFileAlloc(file_path, allocator, std.Io.Limit.limited(1024)) catch continue;
...
pub fn shimRemoveCommand(allocator: std.mem.Allocator, names: []const []const u8) !CommandResult {
    ...
    for (names) |name| {
        ...
        std.Io.Dir.cwd().deleteFile(shim_path) catch |err| {
            if (err == error.FileNotFound) {
