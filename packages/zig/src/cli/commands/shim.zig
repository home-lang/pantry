...
fn createShim(
    ...
    if (!options.force) {
        std.fs.cwd().access(shim_path, .{}) catch |err| {
        ...
        if (std.fs.cwd().access(shim_path, .{})) |_| {
    ...
    // Write shim file
    var file = try std.fs.cwd().createFile(shim_path, .{});
    defer file.close();
...
pub fn shimListCommand(allocator: std.mem.Allocator) !CommandResult {
    ...
    while (try iter.next()) |entry| {
        if (entry.kind == .file) {
            ...
            const content = std.fs.cwd().readFileAlloc(file_path, allocator, std.Io.Limit.limited(1024)) catch continue;
...
pub fn shimRemoveCommand(allocator: std.mem.Allocator, names: []const []const u8) !CommandResult {
    ...
    for (names) |name| {
        ...
        std.fs.cwd().deleteFile(shim_path) catch |err| {
            if (err == error.FileNotFound) {
