...
pub const ShellCommands = struct {
    ...
    pub fn lookup(self: *ShellCommands, pwd: []const u8) !?[]const u8 {
        ...
        while (true) {
            ...
            if (try self.env_cache.get(hash)) |entry| {
                ...
                std.Io.Dir.cwd().access(entry.path, .{}) catch {
                ...
                if (entry.dep_file.len > 0) {
                    const file = std.Io.Dir.cwd().openFile(entry.dep_file, .{}) catch {
    ...
    pub fn activate(self: *ShellCommands, pwd: []const u8) ![]const u8 {
        ...
        if (try self.env_cache.get(project_hash_quick)) |cached_entry| {
            ...
            const current_dep_mtime = if (dep_file) |file| blk: {
                const f = std.Io.Dir.cwd().openFile(file, .{}) catch break :blk 0;
                defer f.close();
        ...
        const env_exists = blk: {
            std.Io.Dir.cwd().access(env_bin, .{}) catch break :blk false;
            break :blk true;
        };
        ...
        if (!env_exists and dep_file != null) {
            ...
            const dep_file_content = std.Io.Dir.cwd().readFileAlloc(
                dep_file.?,
                self.allocator,
            ...
            std.Io.Dir.cwd().makePath(env_dir) catch |err| {
            ...
            const current_dep_mtime = blk: {
                const f = std.Io.Dir.cwd().openFile(dep_file.?, .{}) catch break :blk 0;
                defer f.close();
                const stat = f.stat() catch break :blk 0;
                break :blk @divFloor(stat.mtime.toNanoseconds(), std.time.ns_per_s);
            };
    ...
    fn generateShellCode(self: *ShellCommands, project_root: []const u8, env_dir: []const u8) ![]const u8 {
        ...
        const has_pantry = blk: {
            var dir = std.Io.Dir.cwd().openDir(pantry_bin, .{}) catch break :blk false;
            dir.close();
            break :blk true;
        };
...
test "ShellCommands detectProjectRoot" {
    ...
    {
        const file = try std.Io.Dir.cwd().createFile(pkg_json, .{});
        defer file.close();
        try file.writeAll("{}");
    }
