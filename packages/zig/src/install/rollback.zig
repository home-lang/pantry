...
pub const RollbackManager = struct {
    ...
    pub fn recordFileModified(self: *RollbackManager, path: []const u8) !void {
        ...
        std.Io.Dir.cwd().copyFile(path, std.Io.Dir.cwd(), backup_path, .{}) catch |err| {
        ...
        errdefer {
            ...
            std.Io.Dir.cwd().deleteFile(backup_path) catch {};
        }
        ...
        while (i > 0) {
            ...
            switch (action) {
                .file_created => |path| {
                    ...
                    std.Io.Dir.cwd().deleteFile(path) catch |err| {
                ...
                .file_modified => |info| {
                    ...
                    std.Io.Dir.cwd().deleteFile(info.backup_path) catch {};
                },
                .file_deleted => |path| {
                ...
                .dir_created => |path| {
                    ...
                    std.Io.Dir.cwd().deleteTree(path) catch |err| {
...
test "RollbackManager file creation and rollback" {
    ...
    // Verify file is gone
    const result = std.Io.Dir.cwd().access(test_file, .{});
...
test "RollbackManager directory creation and rollback" {
    ...
    try std.Io.Dir.cwd().makeDir(test_dir);
    ...
    const result = std.Io.Dir.cwd().access(test_dir, .{});
    try std.testing.expectError(error.FileNotFound, result);
}
