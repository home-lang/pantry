...
pub fn validateInstallation(
    ...
    var dir = std.Io.Dir.cwd().openDir(install_path, .{}) catch {
...
pub fn validateBinary(
    ...
    if (@import("builtin").os.tag != .windows) {
        const file = try std.Io.Dir.cwd().openFile(binary_path, .{});
        defer file.close();
...
pub fn validateDirectoryStructure(
    ...
    // Check if install directory exists
    var base_dir = std.Io.Dir.cwd().openDir(install_path, .{}) catch {
        try errors.append(allocator, try std.fmt.allocPrint(
            allocator,
            "Install directory does not exist: {s}",
            .{install_path},
