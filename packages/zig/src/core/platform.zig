...
pub const Paths = struct {
    ...
    pub fn cache(allocator: std.mem.Allocator) ![]const u8 {
        ...
        return switch (Platform.current()) {
            .darwin => try std.fmt.allocPrint(allocator, "{s}/.cache/launchpad", .{home_dir}),
            .linux => blk: {
                // Try XDG_CACHE_HOME first
                if (std.process.getEnvVarOwned(allocator, "XDG_CACHE_HOME")) |xdg| {
                    defer allocator.free(xdg);
                    break :blk try std.fmt.allocPrint(allocator, "{s}/launchpad", .{xdg});
                } else |_| {
                    break :blk try std.fmt.allocPrint(allocator, "{s}/.cache/launchpad", .{home_dir});
                }
            },
            .windows => try std.fmt.allocPrint(allocator, "{s}\\AppData\\Local\\launchpad\\cache", .{home_dir}),
        };
    }
    ...
    pub fn data(allocator: std.mem.Allocator) ![]const u8 {
        ...
        return switch (Platform.current()) {
            .darwin => try std.fmt.allocPrint(allocator, "{s}/.local/share/launchpad", .{home_dir}),
            .linux => blk: {
                ...
                if (std.process.getEnvVarOwned(allocator, "XDG_DATA_HOME")) |xdg| {
                    ...
                    break :blk try std.fmt.allocPrint(allocator, "{s}/.local/share/launchpad", .{home_dir});
                }
            },
            .windows => try std.fmt.allocPrint(allocator, "{s}\\AppData\\Local\\launchpad\\data", .{home_dir}),
        };
    }
    ...
    pub fn config(allocator: std.mem.Allocator) ![]const u8 {
        ...
        return switch (Platform.current()) {
            .darwin => try std.fmt.allocPrint(allocator, "{s}/.config/launchpad", .{home_dir}),
            .linux => blk: {
                ...
                if (std.process.getEnvVarOwned(allocator, "XDG_CONFIG_HOME")) |xdg| {
                    ...
                    break :blk try std.fmt.allocPrint(allocator, "{s}/.config/launchpad", .{home_dir});
                }
            },
            .windows => try std.fmt.allocPrint(allocator, "{s}\\AppData\\Local\\launchpad\\config", .{home_dir}),
        };
    }
...
test "Path resolution" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, cache_dir, "launchpad") != null);

    // Test data directory
    const data_dir = try Paths.data(allocator);
    defer allocator.free(data_dir);
    try std.testing.expect(data_dir.len > 0);
    try std.testing.expect(std.mem.indexOf(u8, data_dir, "launchpad") != null);

    // Test config directory
    const config_dir = try Paths.config(allocator);
    defer allocator.free(config_dir);
    try std.testing.expect(config_dir.len > 0);
    try std.testing.expect(std.mem.indexOf(u8, config_dir, "launchpad") != null);
}
